...
- **자동 형상 관리 의무화**:
  - AI 어시스턴트는 사용자의 명시적인 코드 수정 명령을 수행하고 빌드 검증을 거친 뒤, 사용자의 추가 푸시 요청이 없더라도 **자동으로 변경사항을 Git에 스테이징(add), 한글로 커밋(commit)하고, 리모트에 푸시(push) 및 풀(pull)을 수행**하여 최신 형상을 유지해야 합니다.
  - 커밋 메시지는 **반드시 한글로 작성**해야 하며, Conventional Commits 규칙을 준수하여 작성합니다. (예: `feat: ...`, `fix: ...`, `style: ...`, `docs: ...`)
